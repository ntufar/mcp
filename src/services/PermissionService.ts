...
export class PermissionService {
  ...
  async checkPermission(context: PermissionContext): Promise<PermissionCheckResult> {
    try {
      ...
      return {
        allowed: true,
        userPermissions: userPermissions.permissions as any,
      };
    } catch (error) {
      return {
        allowed: false,
  ...
  private validateContext(context: PermissionContext): { valid: boolean; error?: string } {
  ...
  private async checkFileSystemPermissions(
    context: PermissionContext
  ...
  private async simulateFileSystemPermissionCheck(
    context: PermissionContext
  ): Promise<boolean> {
  ...
  private validateUserPermissions(permissions: UserPermissions): void {
    ...
    for (const key of requiredPermissionKeys) {
      if (typeof permissions.permissions[key as keyof typeof permissions.permissions] !== 'boolean') {
