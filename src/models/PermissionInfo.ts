...
export class PermissionInfo {
  private _data: PermissionInfoData;
  ...
  // Validation
  private validateInput(data: PermissionInfoData): void {
  ...
  private updateFlagsFromMode(mode: string): void {
  ...
  private updateModeFromFlags(): void {
    // Convert boolean flags back to octal mode
    let octal = 0;
    
    if (this._data.readable) octal |= 0o444;
    if (this._data.writable) octal |= 0o222;
    if (this._data.executable) octal |= 0o111;
    
    let specialBits = 0;
    if (this._data.setuid) specialBits |= 0o4;
    if (this._data.setgid) specialBits |= 0o2;
    if (this._data.sticky) specialBits |= 0o1;
  ...
  getOctalMode(): string {
    ...
    if (mode[0] === 'r') octal |= 0o400;
    if (mode[1] === 'w') octal |= 0o200;
    if (mode[2] === 'x') octal |= 0o100;
    
    // Group permissions (bits 5-3)
    if (mode[3] === 'r') octal |= 0o040;
    if (mode[4] === 'w') octal |= 0o020;
    if (mode[5] === 'x') octal |= 0o010;
    
    // Other permissions (bits 2-0)
    if (mode[6] === 'r') octal |= 0o004;
    if (mode[7] === 'w') octal |= 0o002;
    if (mode[8] === 'x') octal |= 0o001;
