...
export class ListDirectoryTool {
  ...
  async* streamDirectoryListing(
    path: string,
    options: Partial<ListDirectoryToolOptions> = {},
    context: {
      userId: string;
      clientId: string;
      clientType: string;
    ...
    try {
      const stream = this.directoryService.streamDirectoryListing(
        path,
        {
          includeHidden: options.includeHidden || false,
          maxDepth: options.maxDepth || 1,
   ...
   */
  private validateOptions(options: ListDirectoryToolOptions): void {
