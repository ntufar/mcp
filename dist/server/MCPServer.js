...
class MCPServer {
    ...
    stats;
    constructor(options = {}) {
        this.config = options.config || Configuration_1.Configuration.createDefault();
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
            ...
            try {
                ...
                this.stats.totalResponseTime += Date.now() - startTime;
                const errorType = error instanceof types_js_1.McpError ? error.code : 'UNKNOWN_ERROR';
                const errorTypeCount = this.stats.errorTypes.get(errorType) || 0;
                this.stats.errorTypes.set(errorType, errorTypeCount + 1);
                // Log error
                await this.auditLoggingService.logFailure('system', 'mcp-server', 'mcp-server', 'tool_call', errorType, error instanceof Error ? error.message : 'Unknown error', Date.now() - startTime, { toolName: request.params.name });
                throw error;
            }
            finally {
                this.stats.activeConnections--;
    ...
    async handleListDirectory(args, context) {
        const result = await this.directoryService.listDirectory(args.path, {
    ...
    async handleReadFile(args, context) {
        const result = await this.fileService.readFile(args.path, {
    ...
    async handleSearchFiles(args, context) {
    ...
    async handleGetFileMetadata(args, context) {
        const result = await this.metadataService.getMetadata(args.path, {
    ...
    async handleCheckPermissions(args, context) {
