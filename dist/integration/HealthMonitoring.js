...
class HealthMonitoring {
    ...
    startTime;
    checkInterval = null;
    constructor(config, fileSystemIntegration, streamingIntegration, auditLoggingService, monitoringConfig) {
        this.config = config;
    ...
    async getHealthStatus() {
        ...
        this.healthHistory = this.healthHistory.filter(h => Date.now() - h.timestamp.getTime() < retentionTime);
        return healthStatus;
    }
    /**
    ...
    getMonitoringStats() {
        ...
        const successfulChecks = this.healthHistory.reduce((sum, h) => sum + h.checks.filter(c => c.status === 'pass').length, 0);
        const failedChecks = this.healthHistory.reduce((sum, h) => sum + h.checks.filter(c => c.status === 'fail').length, 0);
        const averageCheckTime = totalChecks > 0 ?
            this.healthHistory.reduce((sum, h) => sum + h.checks.reduce((s, c) => s + c.duration, 0), 0) / totalChecks : 0;
    ...
    async checkCacheHealth() {
        ...
        try {
            ...
            const totalCacheSize = Object.values(cacheStats).reduce((sum, stats) => sum + (stats.size || 0), 0);
            return {
                name: 'cache_health',
                status: 'pass',
    ...
    determineOverallStatus(checks, metrics, alerts) {
    ...
    async evaluateAlerts(healthStatus) {
        ...
        if (metrics.system.memoryUsage.percentage > this.monitoringConfig.alertThresholds.memoryUsage) {
            this.createAlert('critical', `Memory usage is critical: ${metrics.system.memoryUsage.percentage.toFixed(2)}%`, { metric: 'memory_usage', value: metrics.system.memoryUsage.percentage });
        }
        // CPU usage alert
        if (metrics.system.cpuUsage > this.monitoringConfig.alertThresholds.cpuUsage) {
            this.createAlert('high', `CPU usage is high: ${metrics.system.cpuUsage}%`, { metric: 'cpu_usage', value: metrics.system.cpuUsage });
        }
        // Error rate alert
        if (metrics.application.errorRate > this.monitoringConfig.alertThresholds.errorRate) {
            this.createAlert('critical', `Error rate is critical: ${(metrics.application.errorRate * 100).toFixed(2)}%`, { metric: 'error_rate', value: metrics.application.errorRate });
        }
        // Response time alert
        if (metrics.application.averageResponseTime > this.monitoringConfig.alertThresholds.responseTime) {
            this.createAlert('high', `Response time is high: ${metrics.application.averageResponseTime}ms`, { metric: 'response_time', value: metrics.application.averageResponseTime });
        }
    }
    generateAlertId() {
...
exports.HealthMonitoring = HealthMonitoring;
//# sourceMappingURL=HealthMonitoring.js.map