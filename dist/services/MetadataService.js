...
class MetadataService {
     ...
     */
    async getMetadata(path, options = {}, context) {
        const startTime = Date.now();
        try {
            // Validate path
            const pathValidation = this.pathValidationService.validatePathForOperation(path, 'read');
            if (!pathValidation.isValid) {
                await this.auditLoggingService.logFailure(context.userId, context.clientId, context.clientType, 'get_metadata', pathValidation.errorCode || 'PATH_VALIDATION_FAILED', pathValidation.errorMessage || 'Path validation failed', Date.now() - startTime, { targetPath: path });
                return {
                    path,
                    type: 'unknown',
                    exists: false,
            ...
            if (!permissionCheck.allowed) {
                await this.auditLoggingService.logFailure(context.userId, context.clientId, context.clientType, 'get_metadata', permissionCheck.errorCode || 'PERMISSION_DENIED', permissionCheck.reason || 'Permission denied', Date.now() - startTime, { targetPath: validatedPath });
                return {
                    path: validatedPath,
                    type: 'unknown',
                    exists: true,
            ...
            if (cachedResult && !this.isCacheExpired(cacheKey)) {
                ...
                await this.auditLoggingService.logSuccess(context.userId, context.clientId, context.clientType, 'get_metadata', Date.now() - startTime, { targetPath: validatedPath, cacheHit: true });
                return cachedResult;
            }
            // Get metadata
            ...
            // Log success
            await this.auditLoggingService.logSuccess(context.userId, context.clientId, context.clientType, 'get_metadata', result.executionTime, {
                targetPath: validatedPath,
                type: result.type,
            ...
            await this.auditLoggingService.logFailure(context.userId, context.clientId, context.clientType, 'get_metadata', 'METADATA_ERROR', error instanceof Error ? error.message : 'Unknown error', duration, { targetPath: path });
            return {
                path,
                type: 'unknown',
                exists: false,
    ...
    async getBatchMetadata(paths, options = {}, context) {
        ...
        try {
            ...
            const results = await Promise.all(paths.map(path => this.getMetadata(path, options, context)));
            ...
            // Log batch operation
            await this.auditLoggingService.logSuccess(context.userId, context.clientId, context.clientType, 'get_batch_metadata', batchResult.executionTime, {
            ...
            const duration = Date.now() - startTime;
            await this.auditLoggingService.logFailure(context.userId, context.clientId, context.clientType, 'get_batch_metadata', 'BATCH_METADATA_ERROR', error instanceof Error ? error.message : 'Unknown error', duration, { totalPaths: paths.length });
            throw error;
        }
    }
    /**
    ...
    async getDirectoryMetadata(directoryPath, options = {}, context) {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            await this.auditLoggingService.logFailure(context.userId, context.clientId, context.clientType, 'get_directory_metadata', 'DIRECTORY_METADATA_ERROR', error instanceof Error ? error.message : 'Unknown error', duration, { targetPath: directoryPath });
            throw error;
        }
    }
    /**
    ...
    getMetadataStats() {
        ...
        const responseTimes = []; // Would need to track actual response times
    ...
    // Private methods
    async performMetadataRetrieval(path, options) {
        ...
        try {
            ...
            const mockPermissions = PermissionInfo_1.PermissionInfo.createFromStats(mockStats);
            ...
            if (isDirectory) {
                type = 'directory';
                metadata = Directory_1.Directory.createFromFileSystem(path, mockStats, mockPermissions, Math.floor(Math.random() * 100), Math.floor(Math.random() * 10), Math.floor(Math.random() * 1000000));
            }
            else {
                type = 'file';
                metadata = File_1.File.createFromFileSystem(path, mockStats, mockPermissions, this.getContentType(path), 'utf-8');
            }
            if (isSymlink) {
                type = 'symlink';
                // Add symlink target if requested
                if (options.includeSymlinkTarget) {
                    metadata = File_1.File.createFromFileSystem(path, { ...mockStats, isSymbolicLink: () => true, targetPath: '/target/path' }, mockPermissions, this.getContentType(path), 'utf-8');
                }
            }
            return {
                path,
    ...
    generateMockDirectoryItems(directoryPath) {
        const items = [];
        // Generate mock files
        for (let i = 0; i < 10; i++) {
            items.push({
    ...
    updateStats(path, executionTime, exists) {
        // Update statistics
